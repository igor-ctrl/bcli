...
from bcapi.config._defaults import CONFIG_DIR, CONFIG_FILE, PROJECT_CONFIG_FILE
from bcapi.config._model import BCConfig, BCDefaults, BCProfile
from bcapi.errors import ConfigError


def _find_project_config() -> Path | None:
    """Walk up from CWD looking for .bcapi.toml."""
    current = Path.cwd()
...
def _apply_env_overrides(data: dict) -> dict:
    """Apply BCAPI_* environment variable overrides."""
    env_map = {
...
def load_config() -> BCConfig:
    ...
    2. Project config (.bcapi.toml in CWD or parent)
    3. Environment variables (BCAPI_*)
    """
